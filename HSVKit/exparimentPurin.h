...
#pragma region Imread
	...
	cv::Mat original = cv::imread("pudding/original.png", 0);
	...
	for (int i = 1; i < texnum + 1; i++) {
		...
		if (i <= 9) {
			pTmp = cv::imread("pudding/pudding_p000" + to_string(i) + ".png", 0);
			nTmp = cv::imread("pudding/pudding_n000" + to_string(i) + ".png", 0);
		}
		else if (i <= 99) {
			pTmp = cv::imread("pudding/pudding_p00" + to_string(i) + ".png", 0);
			nTmp = cv::imread("pudding/pudding_n00" + to_string(i) + ".png", 0);
		}
		else {
			pTmp = cv::imread("pudding/pudding_p0" + to_string(i) + ".png", 0);
			nTmp = cv::imread("pudding/pudding_n0" + to_string(i) + ".png", 0);
		}
		cv::flip(pTmp, pTmp, -1);
		cv::flip(nTmp, nTmp, -1);
		original.copyTo(pTex[i - 1]);
		original.copyTo(nTex[i - 1]);
		diffImage(original, pTmp, pTex[i - 1], F, 100.0f);

		diffImage(original, nTmp, nTex[i - 1], F, 100.0f);
...
#pragma region Thread Open
	...
	std::thread thread_process([&] {
		...
		while (thread_process_flag) {
			...
			float diff =clamp((corner_xyz_cam_i[0] - corner_xyz_cam[0]) ,-10.0f,10.0f);
			int idx = abs(diff) * 10;
			idx = clamp(idx, 0, 50);
			//cout << idx<< endl;
			cv::Mat dst;
			if (diff < 0) {
				glfwkit.render(corner_xyz_proj, &img_render, nTex[idx]);
				//dst = nTex[idx];
			}
			else {
