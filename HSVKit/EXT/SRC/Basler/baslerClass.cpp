...
deviceName("")
{
	...
	debug_flag.trigger_in_line = -1; // static_cast<int>(paramTypeBasler::TriggerLineSelector::Line3);
	debug_flag.trigger_out_line = -1;		// static_cast<int>(paramTypeBasler::TriggerLineSelector::Line4);
	debug_flag.trigger_out_lineSource = -1;	// static_cast<int>(paramTypeBasler::TriggerLineSource::ExposureActive);
	debug_flag.trigger_out_invert = false;

	debug_flag.image_reverse = static_cast<int>(paramTypeBasler::ImageReverse::None);
}
...
void basler::parameter_all_print()
{
	...
	if (debug_flag.trigger_in_line == static_cast<int>(paramTypeBasler::TriggerLineSelector::Line3)) {
		baslerMessage(" Trigger In :  Line3");
	}else if(debug_flag.trigger_in_line == static_cast<int>(paramTypeBasler::TriggerLineSelector::Line4)) {
		baslerMessage(" Trigger In :  Line4");
	}
	else {
		baslerMessage(" Trigger In :  None");
	}
	if (debug_flag.trigger_out_line == static_cast<int>(paramTypeBasler::TriggerLineSelector::Line3)) {
		baslerMessage(" Trigger Out :  Line3");
	}
	else if (debug_flag.trigger_out_line == static_cast<int>(paramTypeBasler::TriggerLineSelector::Line4)) {
		baslerMessage(" Trigger Out :  Line4");
	}
	else {
		baslerMessage(" Trigger Out :  None");
	}
	if (debug_flag.trigger_out_lineSource == static_cast<int>(paramTypeBasler::TriggerLineSource::ExposureActive)) {
		baslerMessage(" Trigger Out Source:  ExposureActive");
	}
	else if (debug_flag.trigger_out_lineSource == static_cast<int>(paramTypeBasler::TriggerLineSource::Timer1Active)) {
		baslerMessage(" Trigger Out Source:  Timer1Active");
	}
	else {
		baslerMessage(" Trigger Out Source:  None");
	}
	if (debug_flag.trigger_out_invert) {
		baslerMessage(" Trigger Out Invert:  ON");
	}
	else{
		baslerMessage(" Trigger Out Invert:  OFF");
	}

	if (debug_flag.image_reverse == static_cast<int>(paramTypeBasler::ImageReverse::None)) {
		baslerMessage(" Image Reverse :  None");
	}
	else if (debug_flag.image_reverse == static_cast<int>(paramTypeBasler::ImageReverse::ReverseX)) {
		baslerMessage(" Image Reverse :  ReverseX");
	}
	else if (debug_flag.image_reverse == static_cast<int>(paramTypeBasler::ImageReverse::ReverseY)) {
		baslerMessage(" Image Reverse :  ReverseY");
	}
	else if (debug_flag.image_reverse == static_cast<int>(paramTypeBasler::ImageReverse::ReverseXandY)) {
		baslerMessage(" Image Reverse :  ReverseXandY");
	}
}

/// 個別カメラ接続?
bool basler::connect(int id)
{
	if (id < camera_number)
	{
...
void basler::setParam(const paramTypeCamera::paramInt &pT, const int param)
{
	...
	case paramTypeCamera::paramInt::HEIGHT:
		...
		Cameras->Height.SetValue(height);
		break;
...
{
	setParam(pT, paramTypeBasler::TriggerLineSelector::Line3);
...
/// ver.1.2.0. 追加
void basler::setParam(const paramTypeBasler::AcquisitionMode &pT, const paramTypeBasler::TriggerLineSelector &pTLine)
{
	if (pT == paramTypeBasler::AcquisitionMode::TriggerMode)
	{
		Cameras->AcquisitionFrameRateEnable.SetValue(false);
		if (pTLine == paramTypeBasler::TriggerLineSelector::Line3) {
			Cameras->TriggerSource.SetValue(Basler_UsbCameraParams::TriggerSource_Line3);
		}
		else if (pTLine == paramTypeBasler::TriggerLineSelector::Line4) {
			Cameras->TriggerSource.SetValue(Basler_UsbCameraParams::TriggerSource_Line4);
		}
		debug_flag.trigger_in_line = static_cast<int>(pTLine);
		Cameras->TriggerSelector.SetValue(Basler_UsbCameraParams::TriggerSelector_FrameStart);
		Cameras->TriggerMode.SetIntValue(Basler_UsbCameraParams::TriggerMode_On);
		debug_flag.acquisition_mode = static_cast< int >(paramTypeBasler::AcquisitionMode::TriggerMode);
	}
...
/// 画像の取得(単眼)
int basler::captureFrame(void* data)
{
	if (Cameras->IsGrabbing())
	{
		Cameras->RetrieveResult(timeout, ptrGrabResult, TimeoutHandling_ThrowException);
...
/*
int basler::captureFrameWithIndex(void* data)
{
	if (Cameras->IsGrabbing())
	{
...
/// ver.1.2.0 追加
void basler::setParam(const paramTypeBasler::TriggerLineSelector &pT, const paramTypeBasler::TriggerLineSource &pTs, const bool invert)
{

	Basler_UsbCameraParams::LineSelectorEnums line;
	Basler_UsbCameraParams::LineSourceEnums lineSource;

	debug_flag.trigger_out_line = static_cast<int>(pT);
	debug_flag.trigger_out_lineSource = static_cast<int>(pTs);
	debug_flag.trigger_out_invert = invert;

	switch (pT)
	{
	case paramTypeBasler::TriggerLineSelector::Line3:
		line = Basler_UsbCameraParams::LineSelector_Line3;
		break;
	case paramTypeBasler::TriggerLineSelector::Line4:
	default:
		line = Basler_UsbCameraParams::LineSelector_Line4;
		break;
	}

	switch (pTs)
	{
	case paramTypeBasler::TriggerLineSource::ExposureActive:
		lineSource = Basler_UsbCameraParams::LineSource_ExposureActive;
		break;
	case paramTypeBasler::TriggerLineSource::Timer1Active:
	default:
		lineSource = Basler_UsbCameraParams::LineSource_Timer1Active;
		break;
	}

	Cameras->LineSelector.SetValue(line);
	Cameras->LineMode.SetValue(Basler_UsbCameraParams::LineMode_Output);
	Cameras->LineSource.SetValue(lineSource);
	Cameras->LineInverter.SetValue(invert);

}

void basler::setParam(const paramTypeBasler::ImageReverse &pT) {
	switch (pT)
	{
		case paramTypeBasler::ImageReverse::None:
			Cameras->ReverseX.SetValue(false);
			Cameras->ReverseY.SetValue(false);
		break;
		case paramTypeBasler::ImageReverse::ReverseX:
			Cameras->ReverseX.SetValue(true);
			Cameras->ReverseY.SetValue(false);
			break;
		case paramTypeBasler::ImageReverse::ReverseY:
			Cameras->ReverseX.SetValue(false);
			Cameras->ReverseY.SetValue(true);
			break;
		case paramTypeBasler::ImageReverse::ReverseXandY:
			Cameras->ReverseX.SetValue(true);
			Cameras->ReverseY.SetValue(true);
			break;
		default:
	...
	debug_flag.image_reverse = static_cast<int>(pT);
}