...
#pragma region Imread
	int texnum = 100;
	vector<cv::Mat> pTex(texnum);
	vector<cv::Mat> nTex(texnum);

	cv::Mat origin=cv::imread("girl/original.png", 0);


	cv::Mat original = cv::imread("girl/original.png", 0);
	...
	for (int i = 1; i < texnum + 1; i++) {
		...
		if (i <= 9) {
			pTmp = cv::imread("girl/girl_p000" + to_string(i) + ".png", 0);
			nTmp = cv::imread("girl/girl_n000" + to_string(i) + ".png", 0);
		}
		else if (i <= 99) {
			pTmp = cv::imread("girl/girl_p00" + to_string(i) + ".png", 0);
			nTmp = cv::imread("girl/girl_n00" + to_string(i) + ".png", 0);
		}
		else {
			pTmp = cv::imread("girl/girl_p0" + to_string(i) + ".png", 0);
			nTmp = cv::imread("girl/girl_n0" + to_string(i) + ".png", 0);
		}
		cv::flip(pTmp, pTmp, -1);
		cv::flip(nTmp, nTmp, -1);
		original.copyTo(pTex[i - 1]);
		original.copyTo(nTex[i - 1]);
		diffImage(original, pTmp, pTex[i - 1], F, 50.0f);

		diffImage(original, nTmp, nTex[i - 1], F, 50.0f);

		diffImage(original, original, origin, F, 50.0f);
...
#pragma region Thread Open
	...
	std::thread thread_process([&] {
		...
		while (thread_process_flag) {
			...
			float diff = clamp((corner_xyz_cam_i[0] - corner_xyz_cam[0])/5.0f, -10.0f, 10.0f);
			int idx = abs(diff) * 10;
			int idxj = clamp(idx, 0, 10);
	
			idx = clamp(idx*5, 0, 50);
			//cout << idx<< endl;
			cv::Mat roiA, roiB;
			...
			if (abs(diff) < 0.005f) {
				...
				nTex[idxj](cv::Rect(0, 0, 333, 1024)).copyTo(dst(cv::Rect(0, 0, 333, 1024)));
				nTex[idx](cv::Rect(333, 0, 690, 1024)).copyTo(dst(cv::Rect(333, 0, 690, 1024)));
				
				//dst = nTex[idx];
			}
			else {
				pTex[idxj](cv::Rect(0, 0, 333, 1024)).copyTo(dst(cv::Rect(0, 0, 333, 1024)));
				pTex[idx](cv::Rect(333, 0, 690, 1024)).copyTo(dst(cv::Rect(333, 0, 690, 1024)));
				//glfwkit.render(corner_xyz_proj, &img_render, pTex[idx]);
				//dst = pTex[idx];
			}
