...
float velocityAccel = 1.0f;
float lowpassParam = 0.0f;


int proj_fps=1000;
float maxV = 70.0f;
const string directory = "Expariment/FPS_gausian/"+to_string(proj_fps);
...
void correctHomography(float* h) {
	...
	check1 = cv::imread("check0.png", 0);
	check2 = cv::imread("check1.png", 0);
...
#pragma region Ximea Open
	...
	ximeaCam0.setParam(paramTypeCamera::paramFloat::FPS, 500.0f);
	...
	img_ximea0_cam = cv::Mat(ximea_cam_height, ximea_cam_width, CV_8UC1, cv::Scalar::all(255));
	...
	ximeaCam1.setParam(paramTypeCamera::paramFloat::FPS, 500.0f);
...
#pragma region Thread Open
	...
	std::thread thread_process([&] {
		...
		while (thread_process_flag) {
			...
			for (int i = 0; i < 4; i++) {
				corner_xyz_cam_tmp[3 * i + 0] = corner_xyz_cam[i * 3 + 0];
				corner_xyz_cam_tmp[3 * i + 1] = corner_xyz_cam[i * 3 + 1];
			}
			float accel = (center[0] - 2.0f * pre_center[0] + ppre_center[0]);
			
			if (abs(circumstance) > 1.5f) {
				for (int i = 0; i < 4; i++) {
					//corner_xyz_cam_tmp[3 * i + 0] += (corner_xyz_cam[3 * i + 0] - pre_corner_xyz_cam[3 * i + 0]) * velocityAccel;
					//corner_xyz_cam_tmp[3 * i + 1] += (corner_xyz_cam[3 * i + 1] - pre_corner_xyz_cam[3 * i + 1]) * velocityAccel;
					corner_xyz_cam_tmp[3 * i + 0] += (center[0] - pre_center[0] ) * velocityAccel;
			...
			*/
			...
			for (int i = 0; i < 4; i++) {
				pre_corner_xyz_cam[3 * i + 0] = corner_xyz_cam[i * 3 + 0];
				pre_corner_xyz_cam[3 * i + 1] = corner_xyz_cam[i * 3 + 1];
			}

			//cout << corner_xyz_cam_filtered[0] - pre_corner_xyz_cam_filtered[0] << endl;
			//cout << lowpassParam << endl;

			calib.UndistortPerPoint(corner_xyz_cam_tmp, corner_xyz_undistorted, corner_detected);
			//std::cout << corner_xyz_undistorted[1] << std::endl;
			cvtHomography(corner_xyz_undistorted, corner_xyz_proj, homography);
			/*
			for (int i = 0; i < 4; i++) {
				if (abs(circumstance) > 1.5f) {
					if (circumstance > 0.0f) {
						corner_xyz_proj[3 * i + 0] += 1.0f / proj_width;
					}
					else {
						corner_xyz_proj[3 * i + 0] -= 1.0f / proj_width;
					}
			...
			//cvtHomography(corner_xyz_cam, corner_xyz_proj, homography);
			//cout << corner_xyz_proj[3]<< endl;
			/*
			switch (mspf) {
			case 1:
				texid = 0; break;
			...
			*/
			...
			glfwkit.render(corner_xyz_proj, &img_render, texid);
...
#pragma region Main thread
	...
	while ((key = cv::waitKeyEx(1)) != 'q') {
		if (key == 'h') {
			...
			double sp = std::accumulate(static_psnr.begin(), static_psnr.end(), 0.0) / static_psnr.size();

			double mp = std::accumulate(move_psnr.begin(), move_psnr.end(), 0.0) / move_psnr.size();

			printf("sp:%3.1f mp:%3.1f\n\n", sp, mp);
			...
			homography_proj[cornerkey][0] -= 0.5;
			std::vector<cv::Point2f> src;
			std::vector<cv::Point2f> dst;
			for (int i = 0; i < 4; i++) {
				src.push_back(cv::Point2f(homography_cam[i][0], homography_cam[i][1]));
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					homography[i * 3 + j] = H.at<double>(i, j);
				}
			}
			...
			homography_proj[cornerkey][1] +=0.5;
			std::vector<cv::Point2f> src;
			std::vector<cv::Point2f> dst;
			for (int i = 0; i < 4; i++) {
				src.push_back(cv::Point2f(homography_cam[i][0], homography_cam[i][1]));
