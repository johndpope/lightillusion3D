...
#pragma region Thread Open
	...
	std::thread thread_process([&] {
		...
		while (thread_process_flag) {
			...
			for (int i = 0; i < 4; i++) {
				corner_xyz_cam_tmp[3 * i + 0] = corner_xyz_cam_filtered[i * 3 + 0];
				corner_xyz_cam_tmp[3 * i + 1] = corner_xyz_cam_filtered[i * 3 + 1];
			}
			...
			if (abs(circumstance) > 1.5f) {
				for (int i = 0; i < 4; i++) {
					...
					corner_xyz_cam_tmp[3 * i + 0] += v/60.0f * velocityAccel;
			...
			for (int i = 0; i < 4; i++) {
				pre_corner_xyz_cam_filtered[3 * i + 0] = corner_xyz_cam_filtered[3 * i + 0];
				pre_corner_xyz_cam_filtered[3 * i + 1] = corner_xyz_cam_filtered[3 * i + 1];
			}
